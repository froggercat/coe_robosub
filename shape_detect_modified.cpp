/**
 * @function  shape_detect_v2.cpp
 * @brief		Code to recognize shapes. Added detect_edges_v2()
...
2. Determine contours/vertices
	...
	* Use approxPolyDP to approximate found contours as lines
		* findContours has a lot of noise, but the other line generators should have their own noise reduction algorithms
 ...
 * Functions                          *
 *************************************/
// @author: Darryl McGowan ::: 13/02/08 @1926
...
void detect_edges_v2(Mat img)
{
	...
	// Create binary image using Canny method
	Canny( img, Canny_edges, 125, 350, 3, 1 );
	...
	contour_mat = Mat::zeros( img.size(), CV_8UC3 ); // Initialize matrix to zeros. same size as source img.
                                                            // Assuming 8 bit image with 3 channels 
        // Draw contours in new window

	// arg 3 (if < 0, all contours are drawn)
        // copied arguments 5-8 from general_Contours_demo3.cpp (thickness, linetype, hierachy, maxlevel, offset)
        // http://docs.opencv.org/doc/tutorials/imgproc/shapedescriptors/find_contours/find_contours.html

        drawContours( contour_mat, contours, -1, gen_rand_color( rng ), 1, 8, vector<Vec4i>(), 0, Point() );
    
	// Create new window
	namedWindow("Contours", CV_WINDOW_NORMAL); // CV_WINDOW_AUTOSIZE adjusts window size to fit image    
...
Mat detect_edges_v1(Mat bin_img)
{
    Mat contours;
    vector<Vec2f> lines;
...
/*void contour_draw( Mat source, vector<Point2i> contours, RNG rng) // step 3; FULLY IMPLEMENTED
// @author: Darryl McGowan ::: 13/01/25 @1300
